...
- `models/`: place `sam_vit_h_4b8939.pth` here (download with `bash ../download_models.sh` from the repo root).
...
1. Download the SAM checkpoint locally by running `bash ../download_models.sh` from the repo root (creates `Notebook/models/sam_vit_h_4b8939.pth`).
2. Upload this folder to your Colab VM (e.g., via `!unzip` or `git clone` into `/content/Notebook`).
